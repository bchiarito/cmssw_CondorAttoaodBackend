...
class zttModule(Module):
	...
	self.out.branch("GnMuon2", "I")
        self.out.branch("GMuon_pt2", "F")
        self.out.branch("GMuon_eta2", "F")
        self.out.branch("GMuon_phi2", "F")
        self.out.branch("GMuon_mass2", "F")
        self.out.branch("GMuon_charge2", "F")
        self.out.branch("GMuon_genpt2","F")
        self.out.branch("GMuon_ntrackerlayers2","I")
	...
	for i, muon in enumerate(muons):
                ...
                if PassMuonCuts2:
			if muon != GoodMuon and muon.pt != GoodMuon_pt:
...
#	print("passed transmass cut")
	...
	if HaveGoodTau and pzetaTau: ZInvarMassVTau = self.ZInvarMass(GoodMuon, GoodTau,sf_roc)
	if HaveGoodTP and pzetaTP: ZInvarMassVTP =  self.ZInvarMass(GoodMuon, GoodTwoProng,sf_roc)
	...
	self.out.fillBranch("GnJets",GnJets)	
	...
	if HaveGoodTau and MCTC and pzetaTau: 
	...
	if HaveGoodTP and HaveGoodTau and IsoTP and pzetaTau and pzetaTP:
		...
		if HaveGoodTP and HaveGoodTau and IsoTP == False and pzetaTP and pzetaTau:
			self.out.fillBranch("TPTauDeltaR",deltargood)
	...
	if HaveGoodTP and GoodTwoProng_ntracks==3 and pzetaTP:
...
#	if muon.pt >19 and abs(muon.eta) <2.1 and muon.mediumId and muon.pfRelIso03_all <0.1 and muon.dz < 0.2 and abs(muon.dxy) <0.045: return True
	if muon.pt >25 and abs(muon.eta) <2.1 and muon.mediumId and muon.pfIsoId >=3 and muon.dz < 0.2 and abs(muon.dxy) <0.045: return True
	else: return False
